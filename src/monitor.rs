...
fn directory() -> io::Result<PathBuf> {
    ...
    let check = |path: &Path| -> io::Result<()> {
        ...
        if !meta.is_dir() || meta.uid() != unsafe { libc::getuid() } || meta.mode() & 0o077 != 0 {
            return Err(io::Error::other("Runtime directory must be private and owned by this user"));
        }
        Ok(())
    };
    check(&root)?;
...
pub fn run() -> Result<(), String> {
    ...
    if !locked(&lock) { return Err("A background collector is already running".into()); }
    ...
    let result = (|| {
        while !command::cancelled() {
            ...
            if !continuous { points.clear(); }
            points.push_back(point);
            while points.len() > 61 || points.front().is_some_and(|p| now.saturating_sub(p["time"].as_u64().unwrap_or(0)) > 60_000) {
                points.pop_front();
            }
            let bytes = serde_json::to_vec(&json!({"version":1,"updated":now,"points":points})).map_err(|e| e.to_string())?;
            if bytes.len() as u64 > LIMIT { return Err("Background history exceeds size limit".into()); }
...
fn validate(value: &Value, now: u64) -> bool {
    let Some(updated) = value["updated"].as_u64() else { return false; };
    let Some(points) = value["points"].as_array() else { return false; };
    if value["version"] != 1 || updated > now || now - updated > 3000 || points.is_empty() || points.len() > 61 { return false; }
    let mut previous = None;
    for point in points {
        let Some(time) = point["time"].as_u64() else { return false; };
        if time > updated || updated - time > 60_000 || previous.is_some_and(|p| time <= p) { return false; }
        previous = Some(time);
    }
...
pub fn history() -> Option<Value> {
    ...
    if locked(&lock) { return None; }
    if io::Error::last_os_error().raw_os_error() != Some(libc::EWOULDBLOCK) { return None; }
    let file = OpenOptions::new().read(true).custom_flags(libc::O_NOFOLLOW | libc::O_NONBLOCK)
        .open(dir.join("history.json")).ok()?;
    ...
    let now = now_ms().ok()?;
    if !validate(&value, now) { return None; }
    value["now"] = json!(now);
    Some(value)
}
#[cfg(test)]
mod tests {
    ...
    fn reject_stale_future_and_disordered_history() {
        ...
        let mut bad = good.clone(); bad["version"] = json!(2); assert!(!validate(&bad,2100));
        bad = good.clone(); bad["points"][0]["time"] = json!(2000); assert!(!validate(&bad,2100));
        bad = good; bad["points"] = json!([]); assert!(!validate(&bad,2100));
    }
}
