...
impl BasicSampler {
    pub fn sample(&mut self, now: u64) -> (Value, bool) {
        ...
        let seconds = self
            .monotonic
            .map(|t| instant.duration_since(t).as_secs_f64());
        let boot_delta = self
            .time
        ...
        for (name, total, idle) in &cpus {
            let usage = valid
                .and_then(|_| self.cpus.iter().find(|c| &c.0 == name))
                .and_then(|(_, t, i)| {
                    let dt = total.checked_sub(*t)?;
                    let di = idle.checked_sub(*i)?;
        ...
        let memory = mem();
        point["memory"] = json!(memory["total"]
            .as_f64()
            .filter(|v| *v > 0.0)
            .map(|total| 100.0 * memory["used"].as_f64().unwrap_or(0.0) / total));
        let networks = net();
        let disks = disk();
        for (category, new, old) in [
            ("network", &networks, &self.networks),
            ("disks", &disks, &self.disks),
        ] {
            for (name, (a, b)) in new {
                let prior = old.get(name).zip(valid);
                point[format!("{category}:{name}:first")] =
                    json!(prior.and_then(|((o, _), s)| delta(*a, *o, s)));
                point[format!("{category}:{name}:second")] =
                    json!(prior.and_then(|((_, o), s)| delta(*b, *o, s)));
            }
        }
        self.time = Some(now);
        self.monotonic = Some(instant);
        self.cpus = cpus;
        self.networks = networks;
        self.disks = disks;
        (point, valid.is_some())
