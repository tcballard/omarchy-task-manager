...
dest="$root/dist"
mkdir -p "$dest"
name=omarchy-task-manager-0.1.0
stage=$(mktemp -d)
trap 'rm -rf -- "$stage"' EXIT
mkdir "$stage/$name"
for path in Cargo.toml Cargo.lock rust-toolchain.toml CMakeLists.txt LICENSE README.md FEATURES.md SCOPE.md ARCHITECTURE.md CREDITS.md VERIFICATION.md HARDENING.md HARDENING.sha256 .github src ui packaging tests scripts; do
