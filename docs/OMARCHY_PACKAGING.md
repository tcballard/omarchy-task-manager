# Omarchy packaging for v0.1.0

Task Manager was published to Edge as `0.0.3-3` through [omarchy-pkgs PR #579](https://github.com/omacom/omarchy-pkgs/pull/579). The maintainer requested v0.1.0 followed by promotion beyond Edge and accepted the first non-preview v0.1.0 on 24 September 2026.
...
- Release `0.1.0-1` under the existing `omarchy-task-manager` package name, for x86_64.
...
- Retain the declarative GitHub watch and `allow_prerelease: true`. The historical preview watch remains enabled; v0.1.0 itself is a regular release.
...
1. Merge the v0.1.0 release PR after CI passes. The existing release workflow publishes the tested assets only after both Ubuntu and Arch jobs succeed on main.
...
3. Update the contributor fork's recipe to `0.1.0-1`, remove both backport patches, and apply the fast-ring metadata. Update the existing `omacom/omarchy-pkgs` PR #594.
...
The existing Edge package's successful upstream build and prior XPS use are historical evidence. The maintainer accepted v0.0.5 on the XPS and confirmed logout/reboot startup and GPU checks on 24 September 2026. v0.1.0 preserves that behaviour. No other GPU hardware or ARM acceptance is claimed. ARM is not declared in the recipe. CI and upstream publication results must be recorded against their own commit/run, separately from historical `VERIFICATION.md` and `HARDENING.md` records.
